...
var Engine = (function(global) {
  ...
  function AABBCollisionDetection() {
    // Only checks for collisions if player is on rows 0-3 for slight performance increase
    if (player.y < 4) {

        if(player.y == 0) {
            if(player.x == exitDoor.x) {
                // Player wins
                endGame(true);
            }
            else {
                // Player died in water
                endGame(false);
            }
        }
      ...
      for (let enemy of allEnemies) {
        if (
          enemy.getTrueX() < player.getTrueX() + player.width &&
